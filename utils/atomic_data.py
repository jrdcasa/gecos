...
"""
    * ``atomic_mass`` (dict) -> Example atomic_mass["F"]=19.00
    * ``element_cov_radius`` (dict) --> Example element_cov_radius["H"]=  0.23.\
    Covalent radii in angstroms,supplementary info in: https://jcheminf.biomedcentral.com/articles/10.1186/1758-2946-4-26
    * ``maximal_valences``: Maximum valence for each atom
    * ``element_vdw_truhlar_radius`` (dict) -->  van der Waals radii in angstroms, \
    Mantina et al. J. Phys. Chem. A, Vol. 113, No. 19, 2009, 5806-5812 (Table 12) (https://pubs.acs.org/doi/10.1021/jp8111556)
    * ``distances_revaluated_Okuwaki`` (dict) --> Okuwaki, K. et al. J. Phys. Chem. B 2018, 122, 338−347\
     http://pubs.acs.org/doi/10.1021/acs.jpcb.7b08461
    * ``element_vdw_vmd_radius_bondi`` (dict) -->  van der Waals radii are taken from A. Bondi, J. Phys. Chem., 68, 441 - 452, 1964\
     (https://pubs.acs.org/doi/10.1021/j100785a001),\
    except the value for H, which is taken from R.S. Rowland & R. Taylor, J.Phys.Chem., 100, 7384 - 7391, 1996.\
    /https://pubs.acs.org/doi/10.1021/jp953141%2B)\
    Radii that are not available in ither of these publications have RvdW = 2.00 Å.\
    ...
    * ``element_vdw_vmd_volume_bondi`` (dict) --> Van der Waals volume from element_vdw_vmd_radius_bondi


    Table Okuwaki atom types:
